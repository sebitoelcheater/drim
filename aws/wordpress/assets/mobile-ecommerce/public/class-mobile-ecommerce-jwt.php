...
class Mobile_Ecommerce_Jwt
{
    ...
    public function add_api_routes()
    {
        ...
        register_rest_route($this->namespace, 'token/firebase', [
            'methods' => 'POST',
            'callback' => array($this, 'generate_token_from_firebase'),
        ]);
    ...
    public function generate_token_from_firebase($request)
    {        
        $token = $request->get_param('token');

        if (!$token) {
            return new WP_Error(
                'mobile_ecommerce_no_auth_header',
                __('token is required.', 'wp-api-jwt-auth'),
                array(
                    'status' => 403,
                )
            );
        ...
        if(!$kids) {
            $curl = curl_init($publicKeyURL);
            curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
            $result = curl_exec($curl);
            curl_close($curl);
            $kids = json_decode($result, true);
        }
        ...
        if($token) {
            try {
                ...
                if(isset($decoded->email)) {
                    ...
                    if (!$user) {           
                        return new WP_Error(
                            'mobile_ecommerce_user_not_found',
                            'User not found',
                            array(
                                'status' => 404,
                            )
